...
import static gwc.spock.output.Color.*;
...
class TreePrinter {
	...
	TreePrinter(PrintWriter out, Theme theme, boolean disableColors) {
		this.out = out;
		this.theme = theme;
	...
	private void printVisible(TreeNode node, String indent, boolean continuous) {
		...
		if (node.result().isPresent()) {
			...
			Color resultColor = valueOf(result);
	...
	private String colorCaption(TreeNode node) {
		...
		Color color = node.identifier().map(Color::valueOf).orElse(NONE);
